#!/bin/bash
#
# Polymarket Load Test Script
#
# Required environment variables:
#   COIN_SOURCE_KEY      - Ed25519 private key (hex) for funding accounts
#   ACCOUNT_MINTER_SEED  - Seed for generating test accounts
#
# IMPORTANT: Test accounts must have USD1 collateral before running.
# The buy_outcome transactions require USD1 balance.
#
# Usage: ./run_loadtest.sh [OPTIONS] [RPC_URLS] [DURATION] [MEMPOOL_BACKLOG]
#
# Options:
#   --markets-file FILE    Read market addresses from FILE (one per line)
#
# RPC_URLS can be comma-separated for multiple endpoints:
#   ./run_loadtest.sh "https://node1.example.com/v1,https://node2.example.com/v1" 60 5000
#
# Examples:
#   ./run_loadtest.sh                                    # Uses defaults
#   ./run_loadtest.sh --markets-file markets.txt 60 5000
#   ./run_loadtest.sh "https://node1.example.com/v1,https://node2.example.com/v1" 120 10000
#
...
if [ -z "$COIN_SOURCE_KEY" ]; then
    ...
    echo "  export COIN_SOURCE_KEY=\"0x<your-private-key-hex>\""
    exit 1
fi

if [ -z "$ACCOUNT_MINTER_SEED" ]; then
    echo "ERROR: ACCOUNT_MINTER_SEED environment variable is required"
    echo "  export ACCOUNT_MINTER_SEED=\"your-seed-string\""
    exit 1
fi

# Configuration
# RPC_URLS can be comma-separated for multiple endpoints
RPC_URLS="${1:-https://fullnode.testnet.aptoslabs.com/v1}"
DURATION="${2:-60}"
MEMPOOL_BACKLOG="${3:-100}"
