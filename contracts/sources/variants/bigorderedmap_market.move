/// Multi-Outcome Prediction Market - BigOrderedMap Variant
/// BENCHMARK: Uses BigOrderedMap (B+Tree) instead of Table for registry
/// This is a NEW approach - B+Tree allows parallelism for keys far apart
///
module prediction_market::bigorderedmap_market {
    ...
    use aptos_std::big_ordered_map::{Self, BigOrderedMap};  // <-- CHANGED: Using BigOrderedMap
    ...
    /// BENCHMARK: BigOrderedMap registry (B+Tree - parallelism for distant keys)
    struct MultiMarketRegistry has key {
        markets: BigOrderedMap<address, MarketMetadata>,  // <-- CHANGED: BigOrderedMap
        market_count: u64,
        extend_ref: ExtendRef,
        admin: address,
    }
    ...
    fun init_module(deployer: &signer) {
        let constructor_ref = object::create_named_object(deployer, b"BIGORDEREDMAP_REGISTRY");
        let extend_ref = object::generate_extend_ref(&constructor_ref);

        move_to(deployer, MultiMarketRegistry {
            markets: big_ordered_map::new(),  // <-- CHANGED: BigOrderedMap B+Tree
            market_count: 0,
            extend_ref,
    ...
    public entry fun create_multi_market_with_collateral(
        ...
        // BENCHMARK: BigOrderedMap add (B+Tree insertion O(log N))
        let registry = borrow_global_mut<MultiMarketRegistry>(@prediction_market);
        big_ordered_map::add(&mut registry.markets, market_addr, MarketMetadata {  // <-- CHANGED
            creator: creator_addr,
    ...
    public fun market_exists(market_addr: address): bool acquires MultiMarketRegistry {
        let registry = borrow_global<MultiMarketRegistry>(@prediction_market);
        big_ordered_map::contains(&registry.markets, &market_addr)  // <-- CHANGED
    }
