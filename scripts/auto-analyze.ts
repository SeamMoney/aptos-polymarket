...
function runScript(name: string, command: string): Promise<AnalysisResult> {
...
async function main() {
  ...
  const tpsResult = await runScript(
    'analyze-tps',
  ...
  if (quickMode) {
    ...
    if (fs.existsSync(targetFile)) {
      ...
      const txnsResult = await runScript(
        'analyze-submitted-txns',
        `npx tsx scripts/analyze-submitted-txns.ts ${targetFile}`
      );
    ...
    if (fs.existsSync(deepAnalysisPath)) {
      const deepResult = await runScript(
        'deep-tps-analysis',
        `npx tsx scripts/deep-tps-analysis.ts --minutes ${minutes}`
      );
