...
interface PreflightResult {
  ...
  balances: {
    totalApt: number;
    totalUsd1: number;
    minAptBalance: number;
    minUsd1Balance: number;
  };
  rpc: {
    internal: boolean;
    custom: boolean;
...
async function checkRpcEndpoint(name: string, url: string): Promise<boolean> {
  try {
...
async function checkMarkets(aptos: Aptos, marketAddresses: string[]): Promise<number> {
  let activeCount = 0;
...
async function runPreflight(): Promise<PreflightResult> {
  ...
  const indicesToCheck: { index: number; type: 'amm' | 'transfer' }[] = [];
  ...
  for (const status of accountStatuses) {
    ...
    if (status.hasEnoughApt) result.accounts.aptFunded++;
    if (status.hasEnoughUsd1) result.accounts.usd1Funded++;
    if (!status.hasEnoughApt || !status.hasEnoughUsd1) result.accounts.underfunded++;
