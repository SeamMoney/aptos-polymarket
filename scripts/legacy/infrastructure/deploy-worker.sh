#!/bin/bash
# Deploy HFT worker to a remote server (DigitalOcean, etc.)
# Usage: ./scripts/deploy-worker.sh <server-ip> <worker-id>
#
...
if [ -z "$SERVER_IP" ]; then
  echo "Usage: ./scripts/deploy-worker.sh <server-ip> [worker-id]"
  exit 1
fi

echo "Deploying HFT worker $WORKER_ID to $SERVER_IP..."

# Create deployment package
DEPLOY_DIR="/tmp/hft-deploy-$$"
mkdir -p $DEPLOY_DIR
...
cp scripts/run-demo.sh $DEPLOY_DIR/

# Create worker-specific config
cat > $DEPLOY_DIR/run-worker.sh << 'WORKEREOF'
#!/bin/bash
cd /opt/hft-worker
